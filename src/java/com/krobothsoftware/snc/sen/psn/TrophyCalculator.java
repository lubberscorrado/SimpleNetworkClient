...
public final class TrophyCalculator {
     ...
     * returns an integer, the level is ceilled. i.e.,
     * <p/>
     * <pre>
     * 13.589 = 14 level
     * </pre>
    ...
    /**
     * Gets the progress from giving <code>points</code> of current level.
     *
     * @param points
     * @return progress in percentage
     * @since SEN-PSN 1.0
     */
    public static double getProgress(int points) {
        ...
        if (points < LVL_6) return (dpoints - LVL_5) / (LVL_6 - LVL_5)
                * PROGRESS;
        if (points < LVL_12) return (dpoints - LVL_6) / INCR_12 % 1D
                * PROGRESS;
     ...
     * </pre>
     * <p/>
     * to get the progress, 180(Platinum) must be subtracted from T first
     * <p/>
     * <pre>
     * A / (T - 180) * 100
     * correct expression
     * </pre>
     *
     * @param achievedPoints the achieved points
     * @param totalPoints    the total points
     * @return the game progress
     * @since SEN-PSN 1.0
     */
    public static double getGameProgress(int achievedPoints, int totalPoints) {
     ...
     * @since SEN-PSN 1.0
     */
    public static int getTrophyPoints(int bronze, int silver, int gold,
                                      int platinum) {
        return (platinum * PLATINUM_POINTS) + (gold * GOLD_POINTS)
                + (silver * SILVER_POINTS) + (bronze * BRONZE_POINTS);
    }
     ...
     * @since SEN-PSN 1.0
     */
    public static int getLevelPoints(int level) {
        switch (level) {
            case 0:
            case 1:
                return 0;
     ...
     * @since SEN-PSN 1.0
     */
    public static int getProgressPoints(int level, int progress) {
        double p = progress / 100D;
        switch (level) {
            case 0:
            case 1:
                return (int) (p * LVL_2);
