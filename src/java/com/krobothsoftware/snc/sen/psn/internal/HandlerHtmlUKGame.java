 ...
 * @since SEN-PSN 1.0
 */
public final class HandlerHtmlUKGame extends HandlerSax implements
        ExpressionFilter {
    ...
    public HandlerHtmlUKGame(String psnId) {
        ...
        if (calledStartElement) {
            switch (td) {
                case 1:
                    if (div == 3) {
                        // get platform
                        builder.setPlatform(getPlatform(new String(ch, start,
                                length)));
                        div = -1;
                    }
    ...
    public boolean endElement(int expr, String uri, String localName,
        ...
        if (endTag.equals("table")) throw new StopParseException();

        return true;
    }

    public static Platform getPlatform(String text) {
        text = text.trim();
        if (text.equals("PS3")) return Platform.PS3;
        else if (text.equals("PS Vita")) return Platform.VITA;
        else
